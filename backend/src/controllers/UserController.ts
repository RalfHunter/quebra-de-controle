...
import { User, UserSchemaID, UserSchemaUUID, UserSchemaUUIDString, UserUpdate, UserUpdateIDParam, UserUpdateUUIDParam } from "../utils/validatores/schemas/zod/UserSchema.ts";
...
class UserController {
    ...
    async listUserSecurity(req: Request, res: Response) {
        ...
        try {
            ...
            if (id) {
                const parsedId = UserSchemaUUIDString.safeParse(id)

                if (!parsedId.success) {
                    return res.status(400).json(errorResponse(parsedId.error.issues.map(e => e.message).join(", "), 400))
                }
                // console.log(parsedId.data)
                const data = await this.userService.listUserSecurity(parsedId.data)
